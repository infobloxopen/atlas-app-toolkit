...
func TestGormFiltering(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"field_strings overlaps ['sOmeId', 'egegeg']",
		...
		{
			"not(field_strings overlaps ['sOmeId', 'egegeg'])",
		...
		{
			"field_strings contains ['sOmeId', 'egegeg']",
			"(entities.field_strings @> array[?, ?])",
			[]interface{}{"sOmeId", "egegeg"},
			nil,
			nil,
		...
		{
			"not(field_strings contains ['sOmeId', 'egegeg'])",
			"NOT(entities.field_strings @> array[?, ?])",
			[]interface{}{"sOmeId", "egegeg"},
			nil,
			nil,
